    ...
    for (let i = this.objetos.length - 1; i >= 0; i--) {
      ...
      if (this.objetos[i].choca(this.jugador)) {
        if (this.objetos[i].tipo === 'circulo') {
          ...
          this.velocidadCaida += 0.5; // sube de la velocidad de caída
        } else if (this.objetos[i].tipo === 'rectangulo') {
          this.cuadradosAtrapados++; // sube el contador de cuadrados atrapados
        }
...
// Función para cambiar el estado al juego cuando se apriete cualquier tecla
function keyPressed() {
